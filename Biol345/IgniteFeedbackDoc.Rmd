...
```{r Flow, fig.height=6, fig.width=6}
...
ggplot(Flow_table, aes(x = Flow_opt, y =Flow_results, fill = Flow_opt)) + geom_bar(stat="identity") + 
		...
		theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line=element_line(), legend.position="none", axis.text.x = element_text(angle = 90, hjust = 1)) +
		mytheme + theme(axis.text.x=element_text(angle=45, hjust= 1.1))
...
```{r Paper, fig.height=4, fig.width=4}
...
Paper_table <- data.frame(Paper_opt = as.factor(Paper_opt), Impact_results)
...
```{r Topic, fig.height=5, fig.width=6}
...
```{r Narration, fig.height=3, fig.width=4.5}
Narration_opt <- c("Yes, nicely", "Yes, but rushed", "OK, except/nat the end", "Only at/nthe start")
...
```{r Slides, fig.height=3, fig.width=4}
...
SlideInfo_results <- c(data_table$Slide_Irrel[row_num], data_table$Info_yes[row_num], 
data_table$Info_donotknow[row_num], data_table$Info_some[row_num])
