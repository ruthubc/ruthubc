...
ggplot(aveFile, aes(x = Comp_meas, y = log10(metPopAgeMax))) + 
		...
		scale_shape_discrete("Size needed to disperse") + scale_linetype_discrete("Size needed to disperse") + 
...
ggplot(aveFile, aes(x = any_disp, y = log10(metPopAgeMax))) + 
		...
		ylab("Log metapopulation survival") + 
		xlab("Did any dispersal take place?") + scale_shape_discrete("Dispersal Min Size") + 
...
ggplot(ave_subDispFile, aes(x = as.factor(Comp_meas), y = as.factor(ad_dsp_fd), fill = meanDispBin))  + geom_tile(colour = "white", size=0.1) +
		mytheme + ylab("Adult dispersal size") +  xlab("Competition") + 
		scale_fill_gradient(low="grey83", high="grey18", guide = FALSE)
...
ggplot(subDispFile, aes(x = input_var, y = log10(survivalMean_all))) + 
		...
		geom_smooth(se = FALSE, method = "lm", formula = y ~ x + I(x^2), color = "black", size = 0.5, aes(linetype = as.factor(Comp_meas))) + 
...
ggplot(subDispFile, aes(x = log10(survivalMean_all), y = log10(metPopAgeMax))) + 
		...
		mytheme + ylab("Log max metapopulation age") + 
		geom_smooth(se= FALSE, color = "black", aes(linetype = as.factor(ad_dsp_fd))) +
