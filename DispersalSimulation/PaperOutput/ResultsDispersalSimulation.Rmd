...
varies from 0 to 1. Environmental variance is `r env1`.
...
ggplot(subDispFile, aes(x = log10(survivalMean_all), y = log10(metPopAgeMax))) +
		...
		geom_smooth(se = FALSE, method = "lm", formula = y ~ x/(exp(-x)), color = "black", size = 0.4) +
		scale_y_continuous(breaks=seq(0, 3.0, by=0.5), limits = c(0, 2.7)) +
		scale_x_continuous(breaks=seq(0, 3.0, by=0.5), limits = c(0, 2.7)) + 
		scale_shape_manual("Comp", values=c(0, 1, 2, 3, 18, 20)) + 
...
time. 0 means no disperal took place, 1 disperal did take place. No
...
(@) _Figure [Disp]: Showing how compeition type and minimum adult disperal
size interact to affect whether any dispersal took place. Black squares
indicate no disperal took place, light grey squares indicate disperal occured.
The square with the black outline is the one combination of competition and
minimum disperal size where disperal did take place, but the metapopulation
did not surive to 500 generations.n Environmental variation is set to zero._
...
ggplot(subSubFile , aes(x = input_var, y = log10(survivalMean_all))) + 
		geom_point(size = pointSize, position = position_jitter(w = 0, h = 0), aes(shape = as.factor(Comp_meas))) + 
		geom_smooth(se = FALSE, method = "lm", formula = y ~ x + I(x^2)+ exp(-x), color = "black", size = 0.4, aes(linetype = as.factor(Comp_meas))) + 
		mytheme + ylab("Log population survival") + 
		scale_linetype_discrete("Competition") + 
		scale_shape_discrete("Competition") + 
		xlab("Environmental variation") +
		scale_y_continuous(breaks=seq(0, 3.0, by=0.5), limits = c(0, 2.7)) + 
...
competition type, looking only at minimum adult disperal size 0.6 and 0.8._
...
(@) _Figure [EnvDisp]: Showing how compeition type and minimum adult disperal
size interact to affect whether any dispersal took place with environmental
variation. Black squares indicate no disperal took place, light grey squares
indicate disperal occured and intermediate greys indicate disperal took place
in some metapopulations.  With environmetal variation disperal has gone from
...
(@) _Figure [EnvVarDisp]: Average of log10 number of dispersering colonies per
