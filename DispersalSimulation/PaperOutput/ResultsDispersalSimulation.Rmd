...
```{r MetSurvTotNumDispersers, fig.height=6, fig.width=7}
	


ggplot(subDispFile, aes(x = as.factor(any_disp), y = log10(metPopAgeMax), colour = as.factor(ad_dsp_fd))) +
		mytheme + geom_point(size = pointSize, position = position_jitter(w = 0.05, h = 0.01)) + ylab("Log metapopulation survival") + 
		xlab("Did any dispersal take place?") + scale_colour_discrete("Dispersal Min Size")
...
ggplot(ave_subDispFile, aes(x = as.factor(Comp_meas), y = as.factor(ad_dsp_fd), fill = meanDispBin))  + geom_tile(colour = "white") +
...
---
```{r metapopVar, fig.height=8, fig.width=10}
...
ggplot(subDispFile, aes(x = input_var, y = log10(metPopAgeMax), colour = as.factor(Comp_meas))) + 
		geom_point(size = pointSize, position = position_jitter(w = 0.0, h = 0.01)) + 
		geom_smooth(se = FALSE, method = "lm", formula = y ~ x + I(x^2)) + mytheme + ylab("Log max metapopulation age") + 
		scale_colour_discrete("Competition") + xlab("Environmental variation") +
		scale_y_continuous(breaks=seq(0, 3.0, by=0.5), limits = c(0, 2.7)) + facet_wrap(~ad_dsp_fd)
```
...
Survival against environmental variation 
---
```{r poppopVar, fig.height=8, fig.width=10}

ggplot(subDispFile, aes(x = input_var, y = log10(survivalMean_all), colour = as.factor(Comp_meas))) + 
		geom_point(size = pointSize, position = position_jitter(w = 0, h = 0.05)) + 
		geom_smooth(se = FALSE, method = "lm", formula = y ~ x + I(x^2)) + mytheme + ylab("Log max metapopulation age") + 
		scale_colour_discrete("Competition") + xlab("Environmental variation") +
...
Population Survival vs Metapopulation survival by variance
---

```{r MetSurvVsPopSurvVar, fig.height=8, fig.width=10}

ggplot(subDispFile, aes(x = log10(survivalMean_all), y = log10(metPopAgeMax), colour = as.factor(input_var))) + 
		geom_point(size = pointSize, position = position_jitter(w = 0, h = 0.05)) + 
...
Population Survival vs Metapopulation survival by competition
---

```{r MetSurvVsPopSurvComp, fig.height=8, fig.width=10}

ggplot(subDispFile, aes(x = log10(survivalMean_all), y = log10(metPopAgeMax), colour = as.factor(Comp_meas))) + 
		geom_point(size = pointSize, position = position_jitter(w = 0, h = 0.05)) + 
		mytheme + ylab("Log max metapopulation age") + 
		geom_smooth(se= FALSE) +
		scale_colour_discrete("Competition") + xlab("Population Survival")
		#facet_grid(ad_dsp_fd ~ Comp_meas)
```
...
```{r MetSurvVsPopSurvAdFd, fig.height=8, fig.width=10}

ggplot(subDispFile, aes(x = log10(survivalMean_all), y = log10(metPopAgeMax), colour = as.factor(ad_dsp_fd))) + 
		geom_point(size = pointSize, position = position_jitter(w = 0, h = 0.05)) + 
		mytheme + ylab("Log max metapopulation age") + 
		geom_smooth(se= FALSE) +
		scale_colour_discrete("Adult Dispersal Limit") + xlab("Population Survival")
		#facet_grid(ad_dsp_fd ~ Comp_meas)
```

# Binary dispersal 

```{r MetSurvTotNumDispersersEnv, fig.height=8, fig.width=10}
	
ggplot(subDispFile, aes(x = as.factor(any_disp), y = log10(metPopAgeMax), colour = as.factor(Comp_meas))) +
		mytheme + geom_boxplot() + ylab("Log metapopulation survival") + 
		xlab("") + scale_colour_discrete("Env Variance") + facet_wrap(~input_var)
