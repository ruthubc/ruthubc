...
```{r MetSurvTotNumDispersersEnv, fig.height=4, fig.width=10}
...
```{r WhatRestrictsDispersal, fig.height=2.5, fig.width=4}
...
From comparing figure (above) to figure (other), we can see that disperal has gone from zero to some dispersal when size is 0.6 and competition is 0, size is 0.8, comp is zero,
and when size os 0.8 and comp is 0.2.


Disperal amount and environmental variation
---

```{r EnvVarDispersal, fig.height=3, fig.width=6}
...
ggplot(dispSum, aes(x=input_var, y=all_ave_num_disp)) + 
    ...
    geom_smooth(se = FALSE, color = "black", size = 0.4, method = "loess", span = 0.75) +
    xlab("Environmental variation") + ylab("Number colonies dispersing")
    
 ```
(@) _Figure : Average of log10 number of dispersers per time step_ 
