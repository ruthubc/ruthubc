...
all_sub <- subset(dis_aves, Comp_meas != 0.5 )
all_sub <- subset(all_sub, max_no_off == no_off & input_var == env1)
```
...
ggplot(dis_sub, aes(x = as.factor(Comp_meas), y = as.factor(ad_dsp_fd), fill = MeanMetapopDispBin))  + geom_tile() +
...
disaves_subVar <- subset(disaves_subVar, Comp_meas < 0.5)
...
```{r DispersalAgainstPopSurvivalALL}
	
ggplot(dis_aves, aes(x = as.factor(any_disp), y = log10(pop_age), colour = as.factor(Comp_meas))) +
		mytheme + geom_boxplot() + ylab("Log metapopulation survival") + 
		xlab("") + scale_colour_discrete("Competition")


ggplot(disaves_subVar, aes(x = as.factor(any_disp), y = log10(pop_age), colour = as.factor(Comp_meas))) +
		mytheme + geom_boxplot() + ylab("Log metapopulation survival") + 
		xlab("") + scale_colour_discrete("Competition")
