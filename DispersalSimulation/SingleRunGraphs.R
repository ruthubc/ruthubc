...
theFileName <- "5216_slp1_Rsk0.3_K300_var0.4_dslm0.8_maxOff8"
...
#File <- subset(File, File$colony_ID != 7248 )
...
pngHeight = 480 * (21 +1 )# 400 * number of graphs)
...
interval <- ceiling(max(log(File$num_adsB4_dispersal))/200)

#Histogram of colony sizes

p0 <- ggplot(data = File, aes(x= log10(num_adsB4_dispersal), fill = prevDisp)) + geom_histogram() +  mytheme + ggtitle("histogram of colony sizes")

p00 <- ggplot(data = subset(File, num_ads > 1) , aes(x= log10(num_adsB4_dispersal), fill = prevDisp)) + geom_histogram() +  
...
p4 <- ggplot(data = File, aes(x= colony_age, y = log10(num_ads), colour = prevDisp )) + geom_point() + mytheme + ggtitle("colony size by colony age")

# next size vs dispersers
p5 <- ggplot(data = File, aes(x= log10(num_adsB4_dispersal), y = log10(dispersers))) + geom_point() +  mytheme + stat_smooth(se=FALSE) + 
		scale_y_continuous(limits = c(0, NA)) + ggtitle("nest size before dispersal vs number of dispersers")

p6 <- ggplot(data = File, aes(x= log10(num_adsB4_dispersal), y = pcntDisperse)) + geom_point() + stat_smooth(se = FALSE) + mytheme +
		scale_y_continuous(limits = c(0, 1)) + ggtitle("percentage of ads dispersing")

maxcolsize <- max(log10(File$num_adsB4_dispersal))

p6a <-ggplot(data = subset(File, prevDisp == "now"), aes(x = log10(num_adsB4_dispersal), y = log10(num_ads), colour = prevDisp)) + geom_point() + mytheme +
...
p7 <- ggplot(data = File, aes(x=log10(num_adsB4_dispersal), y = foodPerAd, colour = prevDisp)) + geom_point() + stat_smooth(se = FALSE) +  
...
p8 <- ggplot(data = File, aes(x=log10(num_ads), y = foodPerAd, colour = prevDisp)) + geom_point() + stat_smooth(se = FALSE) +  
		mytheme + scale_y_continuous(limits = c(0, NA)) + ggtitle("num ads after dispersal vs per adult food")

p8a <- ggplot(data = File, aes(x=log10(num_ads), y = foodPerJuv, colour = prevDisp)) + geom_point() + stat_smooth(se = FALSE) +  
		mytheme + scale_y_continuous(limits = c(0, NA)) + ggtitle("Number Juvs per food")


p9 <- ggplot(data = File, aes(x=log10(num_ads), y = AveOffAd, colour = prevDisp )) + geom_point() + stat_smooth(se = FALSE) +  
...
p11 <- ggplot(data = File, aes(x=log10(num_ads), y = pcntMoult, colour = prevDisp)) + stat_smooth(se = FALSE)  + geom_point() +  mytheme +
		scale_y_continuous(limits = c(0, 1)) + ggtitle("percentage juvs moults vs. num ads after dispersal")

p12 <- ggplot(data = File, aes(x=log10(num_ads), y = pot_juv_fd, colour = prevDisp)) + stat_smooth(se = FALSE)  + geom_point() +  mytheme +
		ggtitle("Potential juv food vs colony size")
...
if (nrow(deadcols) == 0){
	...
	p13 <- ggplot(data = subset(File, colAlive== 'dead'), aes(x= log10(num_ads), fill = prevDisp )) + geom_histogram() + mytheme + ggtitle("histogram of size of colonies when died (num ads after dispersal)")
}
rm(deadcols)
...
p16 <- ggplot(data = AdsByPopAgeAndCol, aes(x= log10(num_adsB4_dispersal), y = AdSize, colour = MinMax)) + geom_point() +
...
p17 <- ggplot(data = JuvsByPopAgeAndCol, aes(x= log10(num_adsB4_dispersal), y = JuvSize, colour = MinMax)) + geom_point() +
...
grid.arrange(p0, p00, p1, p2, p3,  p4, p5, p6, p6a, p7, p8, p8a, p9, p11, p12, p13, p16, p17, ncol = 1)

dev.off()

#max(File$num_adsB4_dispersal)

#subFile <- subset(File, log10(num_adsB4_dispersal)<2 & dispersers >1 )
