...
ggplot(Nests, aes(x= NumAds)) + geom_histogram(colour = "white") + mytheme + xlab("Nest Size (# Adult Females)")

```

_Histogram of the size of nests measured_
...
but did not include the instar age by instar sex interaction (table x).
After confirming, not surprisingly, that leg length was highly correlated with instar age
...
Due to this significant interaction we preformed tests on each instar individually. We found that leg length increased with colony size in the older instars, but not significantly so in the younger ones (Table 1, figure 1).
...
did not include instar sex or instar age as fixed effects (table x). Colony size was significant (lmer; $\chi$^2^~`r NSResults[[1]]`,`r NSResults[[2]]`~ =
`r NSResults[[3]]`, p = `r NSResults[[4]]` `r NSResults[[5]]`) with individual condition decreasing as colony size increased (figure 2).
...
sex interaction and the two-way interaction instar age squared by instar sex (figure x).
Colony had a significat effect (glmmPQR; $\chi$^2^~`r NSCondRes$Df`~ =
...
Any interactions of instar age or sex with colony size was not significant.
...
condition peaked at intermediate instar ages and was higher for males (figure 5).
...
`r NSResults[[3]]`, p = `r NSResults[[4]]` `r NSResults[[5]]`, figure 6)
...
Estimating the survival function of single female nests using R's survival pacakge (reference), 
...
_Table x:  The full models used for each test. Varibles printed as (1|*****)
