...
##### Leg length vs logCtFm:InstarNumber interaction
...
Performing post-hoc testing, we found that leg length increased with colony size
in the older instars, but was not significant for younger instars (Table 1,
figure 1).
...
```
The model with the lowest AIC included only one interaction term which was was instar age by colony size interaction and 
did not include instar sex or instar age. Colony size was significant (lmer; $\chi$^2^~`r NSResults[[1]]`,`r NSResults[[2]]`~ =
...
`r IntResults[[3]]`, p = `r IntResults[[4]]` `r IntResults[[5]]`), with condition appearing to decrease faster with colony size as the instars 
...
```{r InstarConditionInteractionGraph, fig.height=2, fig.width=4, dpi=300}
...
ggplot(data = a_predictDF, aes(x = logCtFm, y = lmrPrd, colour = as.factor(InstarNumber))) + geom_line(size = 1) + mytheme +
		ylab("Predicted Condition") + xlab("Log Count Females") + scale_colour_discrete(name = "Instar Age") + theme_bw(base_size=8)
...
insignificant. However only adults had a significant effect, n = `r no_nest`
colonies._
...
There was no significant pattern of within colony variance in leg length and
colony size
...
within-colony condition variance decreased with increasing colony size.
...
`r InstarSexSqrCondRes$stars`, figure 5), with the within-colony variance in
condition peaking at intermediate instar ages and being larger for males.
...
```{r instarCondVarianceGraph, fig.height=3, fig.width=4.5, echo = FALSE, dpi=300}
...
plot + geom_boxplot(data = condBootVar, aes(x = InstarNumber, y = bootVarTrans, group = Instar, fill = InstarSex), outlier.shape = 20, color = "black") +
		mytheme + xlab("Instar Age") + ylab("Variance in Condition") + theme_bw(base_size=10)	
...
Adult female leg length was larger in propagules compared to the source colony
...
(@) _Figure: Condition and leg length of adult females in propagues and their
source colony_
...
to 50 days after establishment.  (figure 5).

```{r PropaguleSurvivalGraph, fig.height=3, fig.width=5, dpi=300}

p1 <- ggsurvplot(prop.survfit,color = "#2E9FDF", break.time.by = 5, 
		xlab = "Days until propagule death", ylab = "Propagule survival probability",  censor = TRUE, 
		font.tickslab = c(8),
		font.x = c(10),
		font.y = c(10))
...
`r nOriginalNests` source colonies._
