...
The model with the lowest AIC included nest size x instar age and the
three-way interaction nest size x instar age x instar sex.   Using this as the
full model and all the terms that include nest size removed from the reduced
model, we found that leg length increases as colony size increases (lmer;
...
```
The model with the lowest AIC included InstarAge, InstarAge, InstarSex, InstarAge squared and instar age squared x instar sex.
...
```
Instar age was significant (lmer; $\chi$^2^~`r Results[[1]]`
...
```
The instar age x sex interaction was also significant (lmer; $\chi$^2^~`r Results[[1]]`
