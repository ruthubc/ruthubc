...
`r NSResults[[3]]`, p = `r NSResults[[4]]` `r NSResults[[5]]`, figure 1), but this effect was only detectable in the older instars (Table 1). 
...
is `r as.character(FullModel@call[2])`. Overall leg length increased with
...
`r as.character(FullModel@call[2])`. Overall condition decreases with colony size
...
(@)  Figure : The results of the linear model showing individual condition of each instar age against colony size with both sexes combined as instar sex was insignificant. However only adults had a significant effect.
...
```
but instar age was significant (lmer; $\chi$^2^~`r IntResults[[1]]` ,`r IntResults[[2]]`~= `r IntResults[[3]]`, p = `r IntResults[[4]]`
`r IntResults[[5]]`).
...
```
In addition, the instar age x sex interaction was also significant (lmer; $\chi$^2^~`r IntSexResults[[1]]`
