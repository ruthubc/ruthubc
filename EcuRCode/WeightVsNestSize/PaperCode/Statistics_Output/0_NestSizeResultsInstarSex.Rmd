...
The model with the the lowest AIC value included the two-way interaction 
...
Using this as the full model we found, not surprisingly, that leg length was
significantly correlated with instar age (lmer; $\chi$^2^~`r InstarResults[[1]]`
...
More notably leg length
increases as colony size increases (lmer; $\chi$^2^~`r NSResults[[1]]`
...
Performing post-hoc tests on each instar individually, we found that leg length
increased with colony size in the older instars, but this tread was not
significant for younger instars (Table 1, figure 1).
...
older instars (there was a significant interaction with instar (p =
...
```
The model with the lowest AIC included only one interaction term, instar age by colony size, and 
...
However, when performing ad-hoc tests on the instars individually we found only
adult condition decreased significantly with colony size (lmer;
...
decreases with colony size (p = `r NSResults[[4]]` `r NSResults[[5]]`) and there
was a significant interaction with instar(p = `r InstarResults[[4]]`
`r InstarResults[[5]]`)._
...
insignificant. However, when testing each instar individually, only adult
...
In addition, there were significant response variable for a model for within-colony variance in condition against colony size.
