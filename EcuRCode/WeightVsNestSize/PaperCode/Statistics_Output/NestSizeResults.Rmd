...
```
The model with the lowest AIC included nest size, instar and instar x nest size interaction. Using this as the full model we found that
...
```
Again the model with the lowest AIC included nest size, instar and instar x nest size interaction.
...
```
  ...
  Figure 2: Individual condition against nest size. The overlaid model is
...
Within Nest Variance Vs. Nest Size
--------------------
...
FullModel <- lmer(relativeVar ~  logCtFm + Instar + logCtFm:Instar  +  (1|NestID)  , data = condVar, REML = FALSE)
...
```
### Condition Variance
The model with the lowest AIC value only included nest size and instar as explantory factors.
Instar was significant (lmer; $\chi$^2^~`r Results[[1]]`
,`r IntResults[[2]]`~= `r IntResults[[3]]`, p = `r IntResults[[4]]`
`r IntResults[[5]]`), however nest size was not (lmer; $\chi$^2^~`r NSResults[[1]]`
,`r NSResults[[2]]`~= `r NSResults[[3]]`, p = `r NSResults[[4]]`
`r NSResults[[5]]`).
...
Figure 3. Condition variance within nests by instar. I am not sure yet whether
this is a real results and/or intereseting to the story

Variance in leg length was not significant for any factors.
...
Figure 4: Condition and leg length of adult females in propagues and their
source nest
...
```{r PropaguleSurvivalGraph, fig.height=3, fig.width=4}
...
Figure 5: The survival function of `r nProps` propagules from `r nOriginalNests`
source nests.