...
myFormula <- condResiduals ~ logCtFm + InstarNumber + InstarNumber:InstarSex + logCtFm:InstarNumber + logCtFm:InstarNumber:InstarSex + (1|NestID)
...
model <- lmer(myFormula, data = dataset, REML = FALSE)
Anova(model)


model  <- lmer(condResiduals ~ logCtFm + InstarNumber + logCtFm:InstarNumber + logCtFm:InstarNumber:InstarSex + (1|NestID), data = dataset, REML = FALSE)

Anova(model)

model  <- lmer(condResiduals ~ logCtFm + InstarNumber + logCtFm:InstarNumber + (1|NestID), data = dataset, REML = FALSE)
...
#FullModel  <- lmer(condResiduals ~ logCtFm + InstarNumber + (1|NestID), data = dataset, REML = FALSE)
...
ggplot(data = a_predictDF, aes(x = logCtFm, y = lmrPrd, colour = as.factor(InstarNumber))) + geom_line(size = 1) + mytheme +
		ylab("Scaled Predicted Condition") + xlab("Colony Size (number of adult females)") + scale_colour_discrete(name = "Instar Age")
