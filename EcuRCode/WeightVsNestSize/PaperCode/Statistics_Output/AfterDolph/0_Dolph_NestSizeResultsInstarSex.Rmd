...
_( $\chi$^2^~`r NSResults[[1]]` ,`r NSResults[[2]]`~= `r NSResults[[3]]`, 
p = `r NSResults[[4]]` `r NSResults[[5]]` )._


_When tested individually the
relationship is only significant for the older instars._
...
interaction between colony size and instar
...
```
The final model only included instar age and colony size. No interaction terms were included.
...
In addition, instar was significant (lmer; $\chi$^2^~`r IntResults[[1]]`,
...
superimposed. Overall condition decreases with colony size 
...
```
There was no significant effect of nest size on within-colony variance in leg length. 
However, the average within-colony uniformity in leg length


`r meanLegBootVar` $\pm$ `r SELegBootVar`, 


which is large given that the range of possible values is between zero and one.
...
# running the model
condBootFormula <-  bootVarTrans ~ logCtFm + InstarNumber:InstarSex + I(InstarNumber^2) + 
...
The average within-colony condition variance was also small at 
...
as within-colony condition variance decreased with
...
InstarSexCondRes <- waldDoc_fun(varBootCondMod, 4:5)
...
InstarSexSqrCondRes <-  waldDoc_fun(varBootCondMod, 6:7)

```

as was instar
age squared crossed with sex 
...
InstarGridGraph(condBootVar, "bootSD_varProp", "Variance in Condition", model = varBootCondMod, same_y_axis = "y")
