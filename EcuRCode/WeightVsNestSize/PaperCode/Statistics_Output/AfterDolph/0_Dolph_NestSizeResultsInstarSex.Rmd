...
condBootVar$condUniform <- 1- condBootVar$bootSD_var
legBootVar$legUniform <- 1- legBootVar$bootSD_var
...
InstarGridGraph(spidersMul, "logLeg", "Leg Length - log transformed", model = FullModelLeg)
...
meanLegBootVar <- round(mean(legBootVar$legUniform),2)
SELegBootVar  <- round(sd(legBootVar$legUniform)/sqrt(length(legBootVar$legUniform)), 2)
...
`r meanLegBootVar` $\pm$ `r SELegBootVar`, which is large given that the range of possible values is between zero and one.

#### Condition variance

```{r CondVariance, results='asis',  echo = FALSE, message = FALSE, warning = FALSE}
# mean and SD
meanCondBootVar <- round(mean(condBootVar$condUniform), 2)
SECondBootVar  <- round(sd(condBootVar$condUniform)/sqrt(length(legBootVar$condUniform)),2)

# formatting the data
totSpis<- sum(condBootVar$N)
condBootVar$lmrWgts <- condBootVar$N/totSpis
condBootVar$bootVarTrans <- asin(sqrt(condBootVar$condUniform))
