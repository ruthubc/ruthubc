...
ModGraphTheme <-theme_bw(base_size=10)  + theme(legend.position="none", plot.title = element_text(vjust=2), 
		axis.title.y = element_text(vjust=0.1, size = 10), axis.title.x = element_text(vjust = 0.95, size = 10), plot.margin=unit(c(0.08, 0.08, -0.05, -0.15),"cm"),  axis.ticks = element_blank(),
...
ggplot(data.frame(x=c(0, 0.97)), aes(x)) +  annotate("rect", xmin = pMin, xmax= pMax, ymin = 0, ymax = 1.02, fill = "light grey") +
		stat_function(fun=CostFun, aes(linetype = "Cost"), size = 0.2) + stat_function(fun = BenFun, aes(linetype = "Benefit"), size = 0.2) +
		...
		annotate("text", x = 0.044, y = 0.32, label = "small\nprey", size = anTxSz, fontface= 'italic') + 
		annotate("text", x = smp, y = 0.27, label = "intermediate prey", size = anTxSz, fontface= 'italic') +
